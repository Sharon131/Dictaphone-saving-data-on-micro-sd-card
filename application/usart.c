...
size_t USART_WriteData(const void *data, size_t dataSize){
    ...
    while(index < dataSize){
        if(USART_PutChar(*(((char*)data)+index))){
            count++;
        } else {
            return count;
        }
...
void USARTx_IRQHandler(void){
	...
	if (__HAL_USART_GET_FLAG(&UartHandle, USART_FLAG_RXNE)) {
		...
		if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_RXNE)) {
			...
			// read the received character and place it in the receive ring buffer
			char received = USARTx -> DR;
    ...
    if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_TXE)) {
        ...
        char toSend;
		// get a character from the transmit ring buffer and send it via UART
		if (RingBuffer_GetChar(&USART_RingBuffer_Tx, &toSend)){
            USARTx->DR = toSend;
        } else {
